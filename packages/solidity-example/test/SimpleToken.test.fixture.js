...
describe('SimpleToken Fixture Tests', async function () {
    ...
    async function deployTokenFixture() {
        ...
        console.log("SimpleToken deployments in fixture:", deployments.map(d => ({
            chain: d.chain,
            address: d.address
        })));
    ...
    describe('transferCrossChain', async function () {
        context('Success Test Cases', async function () {
            it('Should burn the correct amount of tokens for the caller', async function () {
                const { deployments, initialSigners, token0, token1Info } = await loadFixture(deployTokenWithCrossChainFixture);
            ...
            it('Should emit the correct events', async function () {
                const { deployments, initialSigners, token0, token1, token1Info } = await loadFixture(deployTokenWithCrossChainFixture);
        ...
        context('Error Test Cases', async function () {
            it('Should revert when transferring to the zero address', async function () {
                const { token0, token1Info } = await loadFixture(deployTokenWithCrossChainFixture);
            ...
            it('Should revert when transferring amount 0', async function () {
                const { initialSigners, token0, token1Info } = await loadFixture(deployTokenWithCrossChainFixture);
            ...
            it('Should revert when transferring to a nonexistent chain', async function () {
                const { initialSigners, token0 } = await loadFixture(deployTokenWithCrossChainFixture);
            ...
            it('Should revert when no cross chain address is set for target chain', async function () {
                const { initialSigners, token0, token1Info } = await loadFixture(deployTokenFixture); // Use base fixture without cross-chain setup
    ...
    describe('verifySPV', async function () {
        ...
        context('Success Test Cases', async function () {
            it('Should verify a valid SPV proof and return correct values', async function () {
                const { token1, token1Info, origin } = await loadFixture(deployTokenWithSPVFixture);
        ...
        context('Error Test Cases', async function () {
            ...
            it('Should revert if already completed', async function () {
                const { token1, token1Info, origin, receiver, amount } = await loadFixture(deployTokenWithSPVFixture);
    ...
    describe('redeemCrossChain', async function () {
        ...
        context('Success Test Cases', async function () {
            it('Should mint the correct amount of tokens to the receiver', async function () {
                const { token1, receiver, amount, proof } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should emit the correct events', async function () {
                const { token1, origin, receiver, amount, proof } = await loadFixture(deployTokenWithRedeemFixture);
        ...
        context('Error Test Cases', async function () {
            ...
            it('Should revert when redeeming on the wrong chain', async function () {
                const { token0, receiver, amount, proof } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should revert when redeeming on the wrong contract', async function () {
                const { initialSigners, receiver, amount, proof } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should revert when redeeming for wrong receiver', async function () {
                const { token1, initialSigners, receiver, amount, proof } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should revert when redeeming with proof that has been tampered with', async function () {
                const { token1, token1Info, origin, receiver, amount } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should revert if authorized source contract is the zero address', async function () {
                const { token1, token0Info, receiver, amount, proof } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should revert if authorized source contract does not match origin contract address', async function () {
                const { token0, token1, token0Info, receiver, amount, proof } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should revert if redeeming on a chain that is not the target chain Id', async function () {
                const { token0, token1Info, origin, receiver, amount } = await loadFixture(deployTokenWithRedeemFixture);
            ...
            it('Should revert if redeeming for wrong operation type', async function () {
                const { mockToken0, mockToken1, mockToken0Info, mockToken1Info, initialSigners } = await loadFixture(deployMockTokensFixture);
    ...
    describe('FIXTURE ISOLATION TEST', async function () {
        context('Simple Cross-Chain Address Test', async function () {
            it('Should have clean state - no cross-chain addresses set', async function () {
                ...
                const { deployments } = await loadFixture(deployTokenFixture);
                console.log('=== TEST 1: Received deployments:', deployments.map(d => ({ chain: d.chain, address: d.address })));
                ...
                await chainweb.runOverChains(async (chainId) => {
                    ...
                    for (const otherDeployment of deployments) {
                        if (otherDeployment.chain !== chainId) {
                            ...
                            console.log(`Chain ${chainId} -> Chain ${otherDeployment.chain}: ${storedAddress}`);
            ...
            it('Should modify state - set cross-chain addresses', async function () {
                ...
                const { deployments } = await loadFixture(deployTokenFixture);
                console.log('=== TEST 2: Received deployments:', deployments.map(d => ({ chain: d.chain, address: d.address })));
                ...
                await chainweb.runOverChains(async (currentChainId) => {
                    ...
                    for (const targetDeployment of deployments) {
                        if (targetDeployment.chain !== currentChainId) {
                            console.log(`Setting Chain ${currentChainId} -> Chain ${targetDeployment.chain}: ${targetDeployment.address}`);
                            ...
                            // Verify it was set
                            const storedAddress = await currentDeployment.contract.getCrossChainAddress(targetDeployment.chain);
            ...
            it('Should have clean state again - no cross-chain addresses set (CRITICAL FIXTURE TEST)', async function () {
                ...
                const { deployments } = await loadFixture(deployTokenFixture);
                console.log('=== TEST 3: Received deployments:', deployments.map(d => ({ chain: d.chain, address: d.address })));

                console.log('=== TEST 3: Checking clean state again (FIXTURE ISOLATION TEST) ===');
                ...
                await chainweb.runOverChains(async (chainId) => {
                    ...
                    for (const otherDeployment of deployments) {
                        if (otherDeployment.chain !== chainId) {
                            const storedAddress = await deployment.contract.getCrossChainAddress(otherDeployment.chain);
                            console.log(`Chain ${chainId} -> Chain ${otherDeployment.chain}: ${storedAddress} (should be ZeroAddress)`);
                            ...
                            if (storedAddress !== ZeroAddress) {
                                console.error(`❌ FIXTURE ISOLATION FAILED! Previous test state persisted.`);
                                console.error(`   Chain ${chainId} -> Chain ${otherDeployment.chain} should be ${ZeroAddress} but got ${storedAddress}`);
                            } else {
                                console.log(`✅ Chain ${chainId} -> Chain ${otherDeployment.chain} is clean (ZeroAddress)`);
                            }
