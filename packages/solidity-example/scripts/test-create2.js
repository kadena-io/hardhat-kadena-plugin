...
async function main() {
    ...
    console.log(`Network type: ${networkName.includes('hardhat') || networkName.includes('localhost') ? 'local' : 'external'}`);

    // First deployment with salt1
    console.log("\n=== First Deployment ===");
    const saltComponents1 = {
        name: contractName,
        version: "1.0.0",
        deploymentEnv: "test1",
    };
    const saltStr1 = JSON.stringify(saltComponents1);
    const salt1 = ethers.id(saltStr1);
    ...
    try {
        ...
        if (deployed2.deployments.length) {
            const code2 = await ethers.provider.getCode(deployed2.deployments[0].address);
