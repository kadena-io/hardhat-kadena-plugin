...
async function doStuff() {
    ...
    await ensureDirectoryExists(path.join(__dirname, '../artifacts/contracts/'))
...
async function compileContracts(): Promise<CompilerOutput> {
    const solidityFilePath = path.join(__dirname, '../contracts/deterministic-deployment-proxy.yul')
...
async function writeBytecode(bytecode: string) {
    const contractDir = path.join(__dirname, '../artifacts/contracts/deterministic-deployment-proxy.yul/Proxy');
    await ensureDirectoryExists(contractDir);
    const filePath = path.join(contractDir, 'bytecode.txt');
...
async function writeFactoryDeployerTransaction(contract: CompilerOutputContract) {
    const contractDir = path.join(__dirname, '../artifacts/contracts/deterministic-deployment-proxy.yul/Proxy');
    await ensureDirectoryExists(contractDir);
    const filePath = path.join(contractDir, 'deployment.json');
