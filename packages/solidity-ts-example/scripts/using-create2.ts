...
function validateAddresses(
  name: string,
  addresses: Array<{ id: string; address: string; chain: number }>,
) {
    ...
    if (deployments.length > 1) {
      ...
      for (let i = 1; i < deployments.length; i++) {
        if (deployments[i].address !== firstAddress) {
          console.error(
            `❌ ERROR: "${contractType}" has DIFFERENT addresses across chains: ` +
      ...
      if (consistent) {
        console.log(
          `✓ "${contractType}" has CONSISTENT address ${firstAddress} across all chains`,
        );
      }
    }
  }
  ...
  for (const [contractType, deployments] of contractGroups.entries()) {
    ...
    if (addressMap.has(address)) {
      ...
      console.error(
        `❌ ERROR: Different contracts have the same address! "${contractType}" has same address as "${duplicate}": ${address}`,
      );
      hasError = true;
    } else {
  ...
  if (hasError) {
    console.error(
      `❌ ${name} validation failed! This may indicate a security issue.`,
    );
  } else {
...
function validateParameterEffect(
  name: string,
  comparisons: Array<{ param: string; addressA: string; addressB: string }>,
) {
  ...
  for (const { param, addressA, addressB } of comparisons) {
    if (addressA === addressB) {
      console.error(
        `❌ ERROR: Changing "${param}" did NOT change the address (${addressA})!`,
      );
      hasError = true;
    } else {
      console.log(
        `✓ Changing "${param}" correctly results in different address: ${addressA} → ${addressB}`,
      );
    }
  }
  ...
  if (hasError) {
    console.error(
      `❌ Some parameters don't affect the address as expected! This is a security concern.`,
    );
  } else {
    console.log(
      `✓ All parameter changes produce different addresses as expected`,
    );
  }
  return !hasError;
}
...
async function main() {
  ...
  const factoryDeployments: Array<{
    id: string;
    address: string;
    chain: number;
  }> = [];
  ...
  if (defaultDeployment.deployments.length > 0) {
    ...
    defaultDeployment.deployments.forEach((deployment) => {
      tokenDeployments.push({
        id: `Default SimpleToken (chain ${deployment.chain})`,
  ...
  if (customSaltDeployment.deployments.length > 0) {
    ...
    customSaltDeployment.deployments.forEach((deployment) => {
      tokenDeployments.push({
        id: `Custom Salt SimpleToken (chain ${deployment.chain})`,
        address: deployment.address,
        chain: deployment.chain,
      });
  ...
  if (customSignerDeployment.deployments.length > 0) {
    ...
    customSignerDeployment.deployments.forEach((deployment) => {
      tokenDeployments.push({
        id: `Custom Signer SimpleToken (chain ${deployment.chain})`,
        address: deployment.address,
        chain: deployment.chain,
      });
  ...
  if (combinedCustomDeployment.deployments.length > 0) {
    ...
    combinedCustomDeployment.deployments.forEach((deployment) => {
      tokenDeployments.push({
        id: `Combined Custom SimpleToken (chain ${deployment.chain})`,
        address: deployment.address,
        chain: deployment.chain,
      });
  ...
  if (hasDuplicates) {
    console.error(
      '❌ CRITICAL SECURITY ISSUE: Factory and contract deployments have address collisions!',
    );
  } else {
  ...
  console.log(
    '5. Default SimpleToken:',
    defaultDeployment.deployments[0]?.address,
  );
  console.log(
    '6. Custom Salt SimpleToken:',
    customSaltDeployment.deployments[0]?.address,
  );
  console.log(
    '7. Custom Signer SimpleToken:',
    customSignerDeployment.deployments[0]?.address,
  );
  console.log(
    '8. Combined Custom SimpleToken:',
    combinedCustomDeployment.deployments[0]?.address,
  );
}
