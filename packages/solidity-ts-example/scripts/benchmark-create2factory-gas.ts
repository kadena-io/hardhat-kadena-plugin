...
async function main() {
    ...
    const provider = new ethers.JsonRpcProvider('http://127.0.0.1:8545/chain/20/evm/rpc');
    ...
    const create2Artifacts = create2Artifact.contracts['contracts/Create2Factory.sol:Create2Factory'];
    const contractBytecode = '0x' + create2Artifacts.bin;
    const abi = create2Artifacts.abi;

    // Create a ContractFactory
    const create2Factory = new ethers.ContractFactory(abi, contractBytecode, wallet);
    ...
    for (let i = 0; i < numDeployments; i++) {
        ...
        console.log(`- Deployment cost: ${ethers.formatEther(gasUsed * gasPrice)} ETH`);
    }
    ...
    for (const buffer of buffers) {
        ...
        console.log(`With ${Math.round((buffer - 1) * 100)}% buffer at 20 gwei: '${ethers.formatEther(bufferedCost)}' ETH`);
    }

    // Final recommendation with comprehensive explanation
    const recommendedFunding = BigInt(Math.ceil(Number(averageGasUsed * standardGasPrice) * 1.5));
    console.log(`\n=== RECOMMENDED VALUE ===`);
    console.log(`Recommended 'fundingDeployerWith' value: '${ethers.formatEther(recommendedFunding)}' ETH`);
